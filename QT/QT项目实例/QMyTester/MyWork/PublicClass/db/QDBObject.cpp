...
bool QDBObject::executeSql(const QString &sql)
{
    ...
    // 连接信号和槽
    connect(thread, &QThread::started, this, &QDBObject::doExecute);
    connect(&timer, &QTimer::timeout, &loop, &QEventLoop::quit);
    connect(this, &QDBObject::executeFinished, thread, &QThread::quit);
    connect(this, &QDBObject::executeFinished, &loop, &QEventLoop::quit);
...
bool QDBObject::commitRecord(const QSqlQuery &query)
{

    m_sqlQueryObject = std::move(query);
    ...
    // 连接信号和槽
    connect(thread, &QThread::started, this, &QDBObject::doCommit);
    connect(&timer, &QTimer::timeout, &loop, &QEventLoop::quit);
    connect(this, &QDBObject::commitFinished, thread, &QThread::quit);
    connect(this, &QDBObject::commitFinished, &loop, &QEventLoop::quit);
...
void QDBObject::doCommit()
{
    ...
    if (!m_db.isOpen() || !m_sqlQueryObject.isActive()) {
        return;
    }
