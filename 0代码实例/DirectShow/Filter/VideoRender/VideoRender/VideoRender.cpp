...
CVideoRender::CVideoRender(LPUNKNOWN pUnk, HRESULT* phr)
	: CBaseVideoRenderer(CLSID_MyVideoRenderer, FILTER_NAME, pUnk, phr),
	m_hWnd(0), m_converter(0)
{
	m_renderer = new CD2DRenderer();
...
HRESULT CVideoRender::SetVideoWindow(HWND hWnd)
{
	if(!IsWindow(hWnd))
	{
		return E_INVALIDARG;
...
void CVideoRender::SetDisplayMode(DisplayMode displayMode)
{
	m_renderer->SetDisplayMode(displayMode);
}

DisplayMode CVideoRender::GetDisplayMode()
{
